...
type CombinationAux<U extends string, S extends string=U> = 
  U extends string
  ? Exclude<S,U> extends never 
    ? U
    : `${U} ${CombinationAux<Exclude<S,U>>}` | CombinationAux<Exclude<S,U>>
  : never;

type Combination<T extends string[]> = CombinationAux<TupleToUnion<T>>;
